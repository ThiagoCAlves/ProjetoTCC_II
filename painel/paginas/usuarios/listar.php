...
for($i=0; $i<$linhas; $i++){
	...
	<tr style="color:{$classe_ativo}">
		<td><input type="checkbox" id="seletor-{$id}" class="form-check-input" onchange="selecionar('{$id}')">
		{$nome}
		</td>
		<td class="esc">{$telefone}</td>
		<td class="esc">{$email}</td>
		<td class="esc">{$nivel}</td>
		<th class="esc"><img src="images/perfil/{$foto}" width="25px"></a></th>	
		<td>
			<big><a href="#" onclick="editar('{$id}','{$nome}','{$email}','{$telefone}','{$endereco}','{$nivel}')" title="Editar Dados"><i class="fa fa-edit texto primary"></i></a></big>
...
<script type="text/javascript">
	...
	$('#tabela').DataTable({
		"language" : {
			"url" : '//cdn.datatables.net/v/dt/dt-1.13.8/b-2.4.2/b-html5-2.4.2/b-print-2.4.2/datatables.min.js'
		}
	});
	});
</script>
